...
public class VehicleServiceImpl implements VehicleService {
	...
	public VehicleInfo updateVehicle(VehicleInfo vehicle, String chassisNumber) {
		...
		VehicleInfo existingVehicle = vehicleRepo.findById(chassisInt).orElseThrow(() -> new CustomException("Vehicle with this chassis number does not exist."));
	...
	public List<VehicleInfo> getVehiclesByFilter( List<SearchCriteria> searchCriteria) {
		   VehicleSpecification specification = new VehicleSpecification();
	        searchCriteria.stream().map(searchCriterion -> new SearchCriteria(searchCriterion.getKey(),searchCriterion.getOperation(), searchCriterion.getValue())).forEach(specification::add);
	...
	public void writeToCsv(Writer writer)  {
	  ...
	  try( CSVPrinter csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT)) {
	            for (VehicleInfo v : veh) {
	                csvPrinter.printRecord(v.getChassisNumber(), v.getEngineType(), v.getFuelType(), v.getMakeYear(), v.getManufacturer(), v.isElectric(), v.isHybrid());
	            }
